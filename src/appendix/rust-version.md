...
本书基于 `Rust 2021 edition` 编写。请注意，Rust 的稳定编译器版本和 Edition 是两条不同的演进线：截至 2026 年 9 月，最新稳定版为 Rust 1.98.1，Edition 仍通过项目的 `edition` 字段单独选择。

_Cargo.toml_ 中的 `edition` 字段表明代码应该使用哪个版本编译。新项目应显式写出 `edition = "2021"`，这样升级工具链时不会改变项目的语言语义。
